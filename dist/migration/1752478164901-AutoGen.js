...
exports.AutoGen1752478164901 = void 0;
class AutoGen1752478164901 {
    constructor() {
        this.name = 'AutoGen1752478164901';
    }
    up(queryRunner) {
        return __awaiter(this, void 0, void 0, function* () {
            yield queryRunner.query(`CREATE TABLE \`Review\` (\`reviewId\` varchar(36) NOT NULL, \`rating\` int NOT NULL, \`comment\` text NULL, \`reviewType\` enum ('PRODUCT_RATING', 'PARTICIPANT_ATTITUDE_RATING') NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`appId\` varchar(36) NULL, \`reviewerUserId\` varchar(36) NULL, \`reviewedUserId\` varchar(36) NULL, UNIQUE INDEX \`IDX_4f0a34566e56fc16f403df3157\` (\`appId\`, \`reviewerUserId\`, \`reviewedUserId\`), PRIMARY KEY (\`reviewId\`)) ENGINE=InnoDB`);
            yield queryRunner.query(`CREATE TABLE \`Application\` (\`appId\` varchar(36) NOT NULL, \`platform\` enum ('web', 'ios', 'android') NOT NULL DEFAULT 'web', \`status\` enum ('pending', 'accepted', 'rejected') NOT NULL DEFAULT 'pending', \`appliedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`postId\` varchar(36) NULL, \`appUserId\` varchar(36) NULL, UNIQUE INDEX \`IDX_cba5fa33d48553d7ad0a40f938\` (\`postId\`, \`appUserId\`), PRIMARY KEY (\`appId\`)) ENGINE=InnoDB`);
            yield queryRunner.query(`CREATE TABLE \`Post\` (\`postId\` varchar(36) NOT NULL, \`title\` varchar(30) NOT NULL, \`subtitle\` varchar(100) NOT NULL, \`contents\` text NOT NULL, \`status\` enum ('active', 'end', 'expired') NOT NULL DEFAULT 'active', \`period\` int NOT NULL DEFAULT '7', \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`author_user_id\` varchar(36) NULL, PRIMARY KEY (\`postId\`)) ENGINE=InnoDB`);
            yield queryRunner.query(`CREATE TABLE \`Messages\` (\`messageId\` varchar(36) NOT NULL, \`contents\` text NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`readAt\` timestamp NULL, \`senderUserId\` varchar(36) NULL, \`receiverUserId\` varchar(36) NULL, PRIMARY KEY (\`messageId\`)) ENGINE=InnoDB`);
            yield queryRunner.query(`CREATE TABLE \`User\` (\`userId\` varchar(255) NOT NULL, \`email\` varchar(50) NOT NULL, \`password_hash\` varchar(20) NOT NULL, \`nickname\` varchar(20) NOT NULL, \`type\` enum ('INDVIDUALS', 'COMPANIES') NOT NULL DEFAULT 'INDVIDUALS', \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_a0f06a2598032b44cd96e3a26b\` (\`email\`, \`nickname\`), PRIMARY KEY (\`userId\`)) ENGINE=InnoDB`);
            yield queryRunner.query(`ALTER TABLE \`Review\` ADD CONSTRAINT \`FK_a288164d612073c96cbeeb9075c\` FOREIGN KEY (\`appId\`) REFERENCES \`Application\`(\`appId\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
            ...
            yield queryRunner.query(`ALTER TABLE \`Application\` ADD CONSTRAINT \`FK_b9f2941c3f2cb100ef280530e95\` FOREIGN KEY (\`postId\`) REFERENCES \`Post\`(\`postId\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    down(queryRunner) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield queryRunner.query(`ALTER TABLE \`Application\` DROP FOREIGN KEY \`FK_b9f2941c3f2cb100ef280530e95\``);
            ...
            yield queryRunner.query(`ALTER TABLE \`Review\` DROP FOREIGN KEY \`FK_a288164d612073c96cbeeb9075c\``);
            yield queryRunner.query(`DROP INDEX \`IDX_a0f06a2598032b44cd96e3a26b\` ON \`User\``);
            yield queryRunner.query(`DROP TABLE \`User\``);
            yield queryRunner.query(`DROP TABLE \`Messages\``);
            yield queryRunner.query(`DROP TABLE \`Post\``);
            yield queryRunner.query(`DROP INDEX \`IDX_cba5fa33d48553d7ad0a40f938\` ON \`Application\``);
            yield queryRunner.query(`DROP TABLE \`Application\``);
            yield queryRunner.query(`DROP INDEX \`IDX_4f0a34566e56fc16f403df3157\` ON \`Review\``);
            yield queryRunner.query(`DROP TABLE \`Review\``);
        });
    }
}
exports.AutoGen1752478164901 = AutoGen1752478164901;
