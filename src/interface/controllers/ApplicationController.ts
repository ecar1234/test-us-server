...
export class ApplicationController {
    ...
    async getTesterReviews(req: Request, res: Response): Promise<void> {
        try {
            const { appIds } = req.body;
            const reviews = await this.appUseCase.getTesterReviews(appIds);
            res.status(200).json({ status: 200, info: reviews });
        } catch (error) {
            res.status(500).json({ status: 500, error: error.message });
        }
    }
