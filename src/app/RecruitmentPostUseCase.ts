...
export class RecruitmentPostUseCase {
    constructor(private postRepository: RecruitmentPostRepositoryImpl, private userRepository: UserRepositoryImpl, private applicationRepository: ApplicationRepositoryImpl) { }

    async createPost(author: UserEntity, title: string, subtitle: string, platform: string[], contents: string, images: object[], status: string = 'active', period: number = 7): Promise<RecruitmentPostModel> {
        const post = new RecruitmentPostModel(null, author.userId, title, subtitle, platform, contents, status, period, 0, images);
        return this.postRepository.createPost(post);
    }
    async updatePost(id: string, author: UserEntity, title: string, subtitle: string, platform: string[], contents: string, status: string = 'active'): Promise<RecruitmentPostModel> {
    ...
    // }
    async getInitPosts(): Promise<RecruitmentPostModel[][]> {
